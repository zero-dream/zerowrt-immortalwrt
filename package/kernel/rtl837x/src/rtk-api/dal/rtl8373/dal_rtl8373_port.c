...
rtk_uint16 rtl8373_rtct_patch[][2] = {
	{ 0x81A3, 0x2E }, { 0x81A4, 0xE0 }, { 0x81A5, 0x2E }, { 0x81A6, 0xE0 }, { 0x81A8, 0x1D }, { 0x81A9, 0x00 }, { 0x81AF, 0x2D }, { 0x81B0, 0x05 }, { 0x81B2, 0x09 }, { 0x81B3, 0x1F }, { 0x81BC, 0x1D },
	{ 0x81BD, 0x00 }, { 0x81BE, 0x00 }, { 0x81BF, 0xEA }, { 0x81C0, 0x03 }, { 0x81C1, 0xCA }, { 0x81C2, 0x1D }, { 0x81C3, 0x00 }, { 0x81C4, 0x00 }, { 0x81C5, 0x3B }, { 0x81C6, 0x00 }, { 0x81C7, 0x1A },
	{ 0x81C8, 0x00 }, { 0x81C9, 0x54 }, { 0x81CA, 0xFF }, { 0x81CB, 0xD5 }, { 0x81CC, 0x07 }, { 0x81CD, 0xFA }, { 0x81CE, 0xFF }, { 0x81CF, 0x14 }, { 0x81D0, 0x00 }, { 0x81D1, 0x74 }, { 0x81D2, 0xFF },
	{ 0x81D3, 0xDB }, { 0x81D4, 0x09 }, { 0x81D5, 0xF0 }, { 0x81D6, 0xFF }, { 0x81D7, 0x97 }, { 0x81D8, 0xFF }, { 0x81D9, 0xC2 }, { 0x81DA, 0x00 }, { 0x81DB, 0x28 }, { 0x81DC, 0xF1 }, { 0x81DD, 0xA0 },
	{ 0x81DE, 0x00 }, { 0x81DF, 0x94 }, { 0x81E0, 0x00 }, { 0x81E1, 0x76 }, { 0x81E2, 0xFF }, { 0x81E3, 0xB8 }, { 0x81E4, 0xEA }, { 0x81E5, 0xDD }, { 0x81E6, 0x01 }, { 0x81E7, 0x6B }, { 0x81E8, 0xFF },
	{ 0x81E9, 0xAA }, { 0x81EA, 0x00 }, { 0x81EB, 0x06 }, { 0x81EC, 0x05 }, { 0x81ED, 0x0A }, { 0x81EE, 0xFF }, { 0x81EF, 0x9D }, { 0x81F0, 0x00 }, { 0x81F1, 0x10 }, { 0x81F2, 0x00 }, { 0x81F3, 0x05 },
	{ 0x81F4, 0x01 }, { 0x81F5, 0xE0 }, { 0x81F6, 0x00 }, { 0x81F7, 0x00 }, { 0x81F8, 0x00 }, { 0x81F9, 0x00 }, { 0x81FA, 0x00 }, { 0x81FB, 0x00 },
};
...
ret_t dal_rtl8373_portFrcAbility_set(rtk_uint32 port, rtk_port_ability_t *ability)
{
	...
	rtk_uint32 tmp;
	ret_t retVal;
	if ((ability->forcemode >= RTK_ENABLE_END) || (ability->link >= PORT_LINKSTATUS_END) || (ability->duplex >= PORT_DUPLEX_END) || (ability->speed >= PORT_SPEED_END) || (ability->txpause >= RTK_ENABLE_END) ||
	    (ability->rxpause >= RTK_ENABLE_END) || (ability->smi_force_fc >= RTK_ENABLE_END) || (ability->media >= PORT_MEDIA_END))
		return RT_ERR_INPUT;
...
rtk_api_ret_t dal_rtl8373_port_sdsNway_set(rtk_uint32 sdsId, rtk_sds_mode_t sdsMode, rtk_enable_t enable)
{
	...
	case SERDES_HSG:
		if (enable) {
			...
			dal_rtl8373_sds_regbits_write(sdsId, 0, 4, 0x3 << 1,
						      0x3); //force enable
		} else {
...
rtk_api_ret_t dal_rtl8373_sdsMode_set(rtk_uint32 sdsid, rtk_sds_mode_t mode)
{
	...
	if (sdsid == SERDES_ID0) {
		if (mode == SERDES_8221B) {
			...
			cfg_rl6637_sds_mode(UTP_PORT3,
					    1); //here add 8221B phy patch
			...
			rtl8373_setAsicRegBit(RTL8373_SMI_PORT_POLLING_SEL_ADDR, RTL8373_SMI_PORT_POLLING_SEL_SMI_POLLING_SEL3_OFFSET,
					      ENABLED); // RTL8372 set port3  polling internal resolution reg
	...
	if (sdsid == SERDES_ID1) {
		if (mode == SERDES_8221B) {
			...
			cfg_rl6637_sds_mode(UTP_PORT8,
					    1); //here add 8221B phy patch
			SDS_MODE_SET_SW(chiptype, sdsid, SERDES_HSG);
			SDS_MODE_SET_SW(chiptype, sdsid, SERDES_SG);

			//rtl8373_setAsicRegBits(RTL8373_SDS_MODE_SEL_ADDR,RTL8373_SDS_MODE_SEL_SDS1_MODE_SEL_MASK,0x1F);
			rtl8373_setAsicRegBit(RTL8373_SMI_PORT_POLLING_SEL_ADDR, RTL8373_SMI_PORT_POLLING_SEL_SMI_POLLING_SEL8_OFFSET,
	...
	case CHIP_RTL8373N_MODE:
		if (prbs_tx == 31) { //prbs31
			...
			if (on_off == 1) {
				...
				dal_rtl8373_sds_reg_write(sdsid, 0x6, 0x00,
							  0x6080); //[expr ((8<<4)+24576)]
			} else {
	...
	case CHIP_RTL8224N_MODE:
		if (prbs_tx == 31) { //prbs31
			...
			if (on_off == 1) {
				...
				dal_rtl8224_sds_reg_write(sdsid, 0x6, 0x00,
							  0x6080); //[expr ((8<<4)+24576)]
			} else {
...
void SDS_DEBUG_PAD_OUT(rtk_uint32 sdsid, rtk_uint32 chip)
{
	...
	if ((chip == CHIP_RTL8373_MODE) || (chip == CHIP_RTL8372_MODE) || (chip == CHIP_RTL8373N_MODE) || (chip == CHIP_RTL8372N_MODE) || (chip == CHIP_RTL8366U_MODE)) {
		switch (sdsid) {
		case 0:
...
rtk_api_ret_t dal_rtl8373_iol_eye(rtk_uint32 chip, rtk_uint32 sdsid, rtk_uint32 frame)
{
	...
	for (i = 0; i < 64; i++) { //64
		...
		for (j = 0; j < 64; j++) { //64
			...
			//data1 = dump_EYE_DATA(sdsid, 1, chip); //dbg_out Ϊstring
